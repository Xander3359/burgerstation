...
/datum/controller/subsystem/chunk/Initialize()
	...
	wait = initial(wait) //Set the tick rate based on the amount of z-levels.
	if(world.maxz >= 1)
		wait = CEILING(wait/world.maxz,1)

	return TRUE

/datum/controller/subsystem/chunk/on_life()
...
/datum/controller/subsystem/chunk/proc/process_entire_z(z)
