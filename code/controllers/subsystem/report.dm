...
/datum/controller/subsystem/report/unclog(mob/caller)
	wait = -1
	. = ..()

/datum/controller/subsystem/report/Initialize()
	next_report = world.time + PROFILING_DELAY_SECONDS SECONDS
	. = ..()

/datum/controller/subsystem/report/on_life()
...
/datum/controller/subsystem/report/proc/start_profiling()
	world.Profile(PROFILE_RESTART)
	return TRUE

/datum/controller/subsystem/report/proc/end_profling()
