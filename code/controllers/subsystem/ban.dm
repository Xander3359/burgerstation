...
/datum/controller/subsystem/ban/Initialize()
...
/datum/controller/subsystem/ban/proc/is_banned(key, address, computer_id, type)
...
/datum/controller/subsystem/ban/proc/add_ckey_ban(desired_ckey, admin_ckey = "SERVER", reason = "No reason specified.", expires = world.realtime + 86400)
...
/datum/controller/subsystem/ban/proc/remove_ckey_ban(ckey, admin_ckey)
	if(bans_keys[ckey])
		bans_keys -= ckey
