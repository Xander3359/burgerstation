...
/datum/controller/subsystem/area/unclog(mob/caller)
	src.wait = 0
	log_subsystem(src.name,"Shutting down.")
	. = ..()

/datum/controller/subsystem/area/Initialize()
...
/datum/controller/subsystem/area/on_life()
...
/datum/controller/subsystem/area/proc/set_weather(weather_type, enabled = FALSE, list/area/affected_areas)
