...
/datum/controller/subsystem/wikibot/Initialize()
...
/datum/controller/subsystem/wikibot/proc/add_new_wikibot_key(list/keywords, answer)
	wikibot_list += list(list("keywords" = keywords, "answer" = answer))
	rustg_file_write(json_encode(wikibot_list),WIKIBOT)
	return TRUE

/datum/controller/subsystem/wikibot/proc/process_string(asker, string_to_process)
