...
/datum/controller/subsystem/sound/unclog(mob/caller)
...
/datum/controller/subsystem/sound/proc/process_sound(sound/S)
...
/datum/controller/subsystem/sound/on_life()
...
proc/stop_ambient_sounds(mob/M)
	if(!M.client)
		return FALSE
...
proc/stop_music_track(client/hearer)
	var/sound/created_sound = sound()
	created_sound.priority = 100
	created_sound.status = SOUND_MUTE
...
proc/play_ambient_sound(sound_path, list/atom/hearers, volume = 50, pitch = 1, loop = 0, pan = 0, echo = 0, environment = ENVIRONMENT_NONE)
...
proc/play_random_ambient_sound(sound_path, list/atom/hearers, volume = 50, pitch=1, loop=0, pan=0, echo=0, environment=ENVIRONMENT_NONE)
	var/sound/created_sound = sound(sound_path)
...
proc/play_music_track(var/music_track_id, client/hearer, volume=35, looping=FALSE)
